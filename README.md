...
Open the opaque link generated by the Projects API:

```text
/survey/start?entry={encrypted-token}
```

The initial GET validates the opaque token without creating an attempt; its CSRF-protected auto-submit POST then decrypts the survey/user/allocation context, validates access, creates the internal RID and public PID on the backend, records the entry audit, and redirects to a short-lived `/survey/start?journey=...` token bound to that browser session. Link-preview bots therefore do not reserve capacity. The form displays PID only; RID remains internal and neither identifier is trusted as a continuation credential.

The Projects table's **Copy link** button returns this opaque platform pre-screener URL. The provider `entryLink` remains server-side redirect data and is used only after the pre-screener is submitted.
