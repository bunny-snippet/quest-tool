...
# After each current callback, recover at most this many old receipts. Zero is
# an explicit emergency off switch; the hard cap prevents accidental deploy
# configuration from releasing a large backlog storm.
CINT_OPPORTUNITIES_RECOVERY_BATCH = max(
    0,
    min(int(os.getenv("CINT_OPPORTUNITIES_RECOVERY_BATCH", "1")), 10),
)
...
CINT_PROJECT_FILTER_CACHE_INVALIDATION_SECONDS = max(
    0,
    int(os.getenv("CINT_PROJECT_FILTER_CACHE_INVALIDATION_SECONDS", "300")),
)
...
VAULT_CACHE_OPTIONS_INVALIDATION_SECONDS = max(
    1, int(os.getenv("VAULT_CACHE_OPTIONS_INVALIDATION_SECONDS", "120"))
)
...
REPORT_CACHE_DYNAMIC_METADATA_TTL_SECONDS = max(
    1,
    int(os.getenv("REPORT_CACHE_DYNAMIC_METADATA_TTL_SECONDS", "30")),
)
...
if CACHE_ENABLED:
    CACHES = {
        ...
        "reports": {
            "BACKEND": "django.core.cache.backends.redis.RedisCache",
            "LOCATION": os.getenv("REPORTS_REDIS_CACHE_URL", "redis://127.0.0.1:6379/4"),
    ...
    CACHES = {
        ...
        "reports": {
            "BACKEND": "django.core.cache.backends.locmem.LocMemCache",
            "LOCATION": f"{CACHE_KEY_PREFIX}-reports-local",
            "TIMEOUT": REPORT_CACHE_DEFAULT_TTL_SECONDS,
...
VERISOUL_ENV = os.getenv("VERISOUL_ENV", "sandbox").strip().lower()
VERISOUL_PROJECT_ID = os.getenv("VERISOUL_PROJECT_ID", "").strip()
VERISOUL_API_KEY = os.getenv("VERISOUL_API_KEY", "").strip()
VERISOUL_ACCOUNT_SCORE_THRESHOLD = float(os.getenv("VERISOUL_ACCOUNT_SCORE_THRESHOLD", "0.7"))
VERISOUL_TIMEOUT_SECONDS = max(1.0, float(os.getenv("VERISOUL_TIMEOUT_SECONDS", "10")))
VERISOUL_PENDING_TTL_SECONDS = max(10, int(os.getenv("VERISOUL_PENDING_TTL_SECONDS", "30")))
...
ENFORCE_PROJECT_UNIQUE_ENTRY_IP = env_bool(
    "ENFORCE_PROJECT_UNIQUE_ENTRY_IP",
    env_bool("ENFORCE_GLOBAL_UNIQUE_ENTRY_IP", True),
)
...
CLIENT_INTEGRATION_RFG_SYNC_INTERVAL_SECONDS = max(
    600, int(os.getenv("CLIENT_INTEGRATION_RFG_SYNC_INTERVAL_SECONDS", "600"))
)
CLIENT_INTEGRATION_CINT_SYNC_INTERVAL_SECONDS = int(os.getenv("CLIENT_INTEGRATION_CINT_SYNC_INTERVAL_SECONDS", "60"))
CLIENT_INTEGRATION_PURESPECTRUM_SYNC_INTERVAL_SECONDS = int(os.getenv("CLIENT_INTEGRATION_PURESPECTRUM_SYNC_INTERVAL_SECONDS", "60"))
...
RFG_LOG_RECONCILE_BATCH = max(1, int(os.getenv("RFG_LOG_RECONCILE_BATCH", "50")))
