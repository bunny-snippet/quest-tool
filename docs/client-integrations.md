...
Cint respondent traffic resolves `CINT_API_KEY` and `CINT_HASH_KEY` only from environment variables. Detail refresh first retrieves an existing supplier link and, when missing, creates an OWS/NONE link. After inventory synchronization, a serialized background job calls `SupplierLinks/Update/{SurveyNumber}/{SupplierCode}` for every newly discovered survey and configures the platform's four MID-based status redirects. A callback-contract fingerprint is stored in provider-local survey metadata and preserved across inventory updates, so already configured surveys are skipped. Existing inventory can be configured once with `python manage.py sync_cint_redirects`; batches are queued through Celery by default. The final entry URL uses the vault UID as PID, the unique attempt RID as MID, the normalized SHA-256 hash of a real email permanently assigned from the encrypted respondent pool, provider question-ID/Precode pairs, and an HMAC-SHA1 URL signature appended last. Employee login emails and plaintext respondent emails are never sent. `PUBLIC_APP_BASE_URL` must be the public HTTPS origin used for callbacks. Profile reuse remains disabled (`profile_reuse_enabled=false`, percentage `0`) unless a separately audited same-respondent policy is enabled.
