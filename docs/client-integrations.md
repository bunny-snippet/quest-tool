...
Cint respondent traffic resolves `CINT_API_KEY` and `CINT_HASH_KEY` only from environment variables. Inventory is fetched only from `AllOfferwall/ByCountryLanguage/{CountryLanguageID}/{SupplierCode}` for 6/ENG-CA, 7/ENG-IN, 8/ENG-GB, 9/ENG-US, 10/FRE-FR and 76/HIN-IN. CA, IN, US and FR accept inclusive RPI 1-4; GB accepts inclusive RPI 1-2. Rejected rows never enter the database. For an accepted new row the platform POSTs `SupplierLinks/Create`; for a changed existing row it PUTs `SupplierLinks/Update`, using the OWS/NONE MID callback contract. The row is saved only after the upstream supplier-link call succeeds. A callback-contract fingerprint prevents duplicate PUTs for unchanged rows. The final entry URL uses the vault UID as PID, the unique attempt RID as MID, the normalized SHA-256 hash of a real email permanently assigned from the encrypted respondent pool, provider question-ID/Precode pairs, and an HMAC-SHA1 URL signature appended last. Employee login emails and plaintext respondent emails are never sent. `PUBLIC_APP_BASE_URL` must be the public HTTPS origin used for callbacks. Profile reuse remains disabled (`profile_reuse_enabled=false`, percentage `0`) unless a separately audited same-respondent policy is enabled.
